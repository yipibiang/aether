 ...
 * <h3>JDK21 特性使用</h3>
 * <ul>
 *   <li>{@code var} — JDK10 局部变量类型推断，编译器自动推断类型</li>
 *   <li>{@code record} — JDK16 不可变数据类，见 {@link ToolResult}</li>
 *   <li>{@code switch} 表达式 + 模式匹配 — 见 {@link #deriveProgress}</li>
 *   <li>{@code Stream.toList()} — JDK16 便捷收集器</li>
 *   <li>{@code List.of()} / {@code Map.of()} — JDK9 不可变集合工厂</li>
 * </ul>
 *
 * <h3>响应式流 (Reactor)</h3>
 * 使用 {@link Flux} 实现流式事件推送，UI 层订阅事件进行渲染。
 * {@link FluxSink} 是 Reactor 的手动发射器，类似 JDK9 Flow API 的 Publisher。
 */
public class ReActAgent implements Agent, AgentMiddleware.AgentContextView {
    ...
    private List<SkillFrontmatter> skills = List.of();
    ...
    @Override
    public List<SkillFrontmatter> skills() { return skills; }

    @Override
    public String requestedSkillName() { return requestedSkillName; }

    public void setSkills(List<SkillFrontmatter> skills) { this.skills = skills; }
    ...
    private CompletableFuture<Void> runBeforeAgentRun() {
        return runMiddlewareChainVoid(mw -> mw.beforeAgentRun(this));
    }

    private CompletableFuture<Void> runAfterAgentRun() {
        return runMiddlewareChainVoid(mw -> mw.afterAgentRun(this));
    }

    private CompletableFuture<Void> runBeforeAgentStep(int step) {
        return runMiddlewareChainVoid(mw -> mw.beforeAgentStep(this, step));
    }

    private CompletableFuture<Void> runAfterAgentStep(int step) {
        return runMiddlewareChainVoid(mw -> mw.afterAgentStep(this, step));
    }
    ...
    /**
     * JDK16 record：工具执行结果，不可变数据载体。
     * 等价于 JDK8 中手写 final class + 全参构造器 + getter。
     */
